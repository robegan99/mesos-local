;(function ($, window, document, undefined) {
  ...
  Plugin.prototype.init = function () {
    ...
    if(this.carousel_slides.length <= 1) {
      ...
      this.carousel_children.each(function() {
  ...
  Plugin.prototype.carousel_timeout_start = function() {

    var self = this;

    this.carousel_timeout_clock = setTimeout((function() {

      self.element.addClass('is-set');
  ...
  Plugin.prototype.carousel_slide_animate_to = function(index) {
    ...
    if ((this.carousel_slides.length == "1") || (index == this.carousel_slides_index_active)) {

      return false;

    }

    this.element.removeClass('is-set');

    if(index < this.carousel_slides_index_active) {

      this.element.addClass("is-reversing");

    } else {

      this.element.removeClass("is-reversing");
  ...
  Plugin.prototype.carousel_slide_update_neighbors = function() {
    ...
    // Loop Over Elements and Adjust Order

    var order_index = 2;
    var step_index = this.carousel_slides_index_active;

    do {

      if (step_index == this.carousel_slides_index_previous) {

        this.carousel_slides[step_index].addClass("is-leaving");
        this.carousel_slides[step_index].css('order', 1);

      } else {

        this.carousel_slides[step_index].removeClass("is-leaving");
        this.carousel_slides[step_index].css('order', order_index);
        order_index++;
      ...
      if (step_index >= this.carousel_slides_count) {

        step_index = 0;

      }

    } while (step_index != this.carousel_slides_index_active);
